...
def _sleep_before_retry(
    *,
    ...
    _sleep_with_cancel(delay, cancel_event=cancel_event)


def _sleep_with_cancel(delay_seconds: float, *, cancel_event: CancelEvent = None) -> None:
    if delay_seconds <= 0:
        return
...
def _post_json_once(
    *,
    ...
    return _parse_json_response(_run_command(command, cancel_event=cancel_event, runner=runner))
...
def request_generation(
    *,
    ...
    return _request_image_operation(
        ...
        retry_label="请求接口",
        request_once=lambda: _post_json_once(
            url=url,
            headers=headers,
            payload=payload,
            config=config,
