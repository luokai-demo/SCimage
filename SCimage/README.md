...
- 自动 tag 格式为 `v<版本号>`，例如 `v1.1.3`
- 如果同名版本 tag 已经存在，自动打标签会失败并提示先提升 `VERSION`
- 新 tag 会自动触发桌面打包工作流
...
- 另存为新配置
- 下拉切换已保存配置
- 拉取模型列表，也支持直接手动输入模型 ID
- 对不支持 `/models` 或权限不足的 API，模型列表提示会作为非阻断提醒展示
...
- 全局滚动条策略改为默认隐藏，减少小面板和横向拖拽区出现滚动条遮挡
- 任务列表、图库窗口、模型列表和提示词列表保留优化后的可见滚动条，适合大量数据浏览
- 图生图顶部族谱栏继续支持横向拖拽滑动，但隐藏可见滚动条
- 修复族谱栏点击无法切换的问题，拖拽和点击不再互相干扰
- 补充族谱根图栏回归测试，覆盖拖动、点击和误触防护
- Release 正文改为读取版本专属中文说明文件，避免自动发布后再单独补更新说明

## v1.1.2 重点

- 删除低清占位预览链路，图库和族谱统一使用原图展示，避免根图或卡片先显示模糊预览
- 清理历史任务、图库分页和族谱数据中的 `preview_url`、占位色等旧字段，后端启动时会迁移旧索引表结构
- API 配置在已有参数时默认收起，减少左侧工作区被配置表单占用
- 模型选择改为“手动输入 + 下拉选择”组合控件，下拉菜单悬浮到顶层显示，避免被配置卡片裁剪
- 模型列表获取失败不再作为红色阻断错误；不支持 `/models`、余额不足或权限不足时仍可直接手动输入模型 ID
- 提供方配置保存取消对 `/models` 返回列表的强制校验，保留 Base URL 规范化和 API Key 继承逻辑
- 任务队列和图生图族谱回归用例同步更新，覆盖手动模型、图片模型排序、无低清预览和旧数据迁移
